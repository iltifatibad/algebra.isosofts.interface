...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/leg/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch(`/api/register/leg/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
...
const archiveData = (id) => {
  ...
  if (showArchived) {
    fetch(`/api/register/leg/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ids: [...selectedRows],
    ...
    fetch(`/api/register/leg/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
<div className="flex items-center gap-4 flex-wrap">
  ...
  >
    ...
    {!showAction ? "Add Legislation" : "Add Action"}
  </button>
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        ...
        <div className="px-8 py-6">
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-5">
              <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest">Legislation Details</p>
              ...
              {/* Text Inputs */}
              {[
                { label: "Legislation", field: "legislation", placeholder: "Enter legislation..." },
                { label: "Section", field: "section", placeholder: "Enter section..." },
                { label: "Requirement", field: "requirement", placeholder: "Enter requirement..." },
                { label: "Risk Of Violation", field: "riskOfViolation", placeholder: "Enter risk of violation..." },
              ...
              {/* Affected Position */}
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Affected Position</label>
                <select
                  value={formData.affectedPosition}
                  onChange={(e) => handleFormChange("affectedPosition", e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
        ...
        <div className="px-8 py-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50 rounded-b-2xl">
          ...
          <button onClick={saveRisk} className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl hover:from-blue-600 hover:to-blue-800 shadow-sm shadow-blue-200 transition-all">
            {modalMode === "add" ? "Add Legislation" : "Update Legislation"}
          </button>
        </div>
      </div>
    </div>
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={legHelpContent} />
    </div>
  );
};
