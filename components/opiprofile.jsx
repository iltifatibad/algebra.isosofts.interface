...
import { opiHelpContent } from "./utils/helpContents.js";
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      if (!showAction) {
        ...
        lykpi: parseInt(formData.lykpi),
        actualKPI: parseInt(formData.actualKPI),
        annualTarget: parseInt(formData.annualTarget),
        january: parseInt(formData.january),
        february: parseInt(formData.february),
        march: parseInt(formData.march),
        april: parseInt(formData.april),
        may: parseInt(formData.may),
        june: parseInt(formData.june),
        july: parseInt(formData.july),
        august: parseInt(formData.august),
        september: parseInt(formData.september),
        october: parseInt(formData.october),
        november: parseInt(formData.november),
        december: parseInt(formData.december),
        };
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={opiHelpContent} />
    </div>
  );
};
