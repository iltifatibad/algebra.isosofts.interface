...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/moc/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch(`/api/register/moc/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
...
const archiveData = (id) => {
  ...
  if (showArchived) {
    fetch(`/api/register/moc/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ids: [...selectedRows],
    ...
    fetch(`/api/register/moc/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        ...
        <div className="px-8 py-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-800">
              {modalMode === "add" ? "Add New MoC" : "Edit MoC"}
            </h3>
          </div>
        ...
        <div className="px-8 py-6 space-y-6">
          <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest">MoC Details</p>
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Reason Of Change</label>
                <input
                  value={formData.reasonOfChange}
                  onChange={(e) => handleFormChange("reasonOfChange", e.target.value)}
                  type="text"
    ...
    {(() => {
      ...
      return (
        ...
        <div className="px-8 py-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50 rounded-b-2xl">
          ...
          >
            {modalMode === "add" ? "Add MoC" : "Update MoC"}
          </button>
        </div>
      </div>
    </div>
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={mocHelpContent} />
    </div>
  );
};
