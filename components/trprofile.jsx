...
const TrProfile = () => {
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      if (!showAction) {
        ...
        fetch("http://localhost:8000/api/register/tra/one", {
          method: "POST",
      ...
      if (!showAction) {
        ...
        console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
        const url =
          "http://localhost:8000/api/register/tra/one/" + selectedTable[0].id;
        fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload), // Direkt obje – array yapma!
  ...
  const handleDeleteConfirm = () => {
    if (activeHeader) {
      if (!showDeleted) {
        fetch("http://localhost:8000/api/register/tra/all/delete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
        ...
        fetch("http://localhost:8000/api/register/tra/all/undelete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
  ...
  const archiveData = (id) => {
    if (showArchived) {
      fetch("http://localhost:8000/api/register/tra/all/unarchive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch("http://localhost:8000/api/register/tra/all/archive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
