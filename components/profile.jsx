...
const RisksAssessment = () => {
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      ...
      if (!showAction) {
        ...
        console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
        const url =
          "/api/register/br/one/" + selectedTable[0].id;
        fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload), // Direkt obje – array yapma!
  ...
  const handleDeleteConfirm = () => {
    if (activeHeader) {
      if (!showDeleted) {
        fetch("/api/register/br/all/delete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
        ...
        fetch("/api/register/br/all/undelete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
  ...
  const archiveData = (id) => {
    if (showArchived) {
      fetch("/api/register/br/all/unarchive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch("/api/register/br/all/archive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
                ...
                {[
                  ...
                  <div key={field} className="group">
                    ...
                    <input
                      ...
                      onChange={(e) => handleFormChange(`actionPlan[0].${field}`, field === "resources" ? parseInt(e.target.value) : e.target.value)}
                      type={type}
