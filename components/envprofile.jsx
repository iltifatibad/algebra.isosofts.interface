...
const saveRisk = () => {
    ...
    if (modalMode === "add") {
        ...
        if (!showAction) {
            ...
            console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
            const url =
                `/api/register/eai/one/${selectedTable[0].id}?token=${token}`;
            fetch(url, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload), // Direkt obje – array yapma!
...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/eai/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch(`/api/register/eai/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
...
const archiveData = (id) => {
  ...
  if (showArchived) {
    fetch(`/api/register/eai/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ids: [...selectedRows],
    ...
    fetch(`/api/register/eai/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
            ...
            <div className="space-y-6">
              ...
              <div className="group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    console.log("Select onChange tetiklendi! Yeni value:", e.target.value);
                    handleFormChange("aspect", e.target.value);
                  }}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
              ...
              <div className="group">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Enter Existing Control Measures..."
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                />
              </div>
