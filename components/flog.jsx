...
import FBody from "./tabledatas/frisk.jsx";
import FHeaders from "./tableheaders/fheaders.jsx";

import ReactECharts from "echarts-for-react";
import HelpModal from "./utils/HelpModal.jsx";
import ExportButton from "./utils/ExportButton.jsx";
import { flogHelpContent } from "./utils/helpContents.js";
...
const saveRisk = () => {
    ...
    if (modalMode === "add") {
        ...
        if (!showAction) {
            ...
            console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
            const url =
                `/api/register/fin/one/${selectedTable[0].id}?token=${token}`;
            fetch(url, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload), // Direkt obje – array yapma!
        ...
        const payload = {
          ...
          december: actionData.actionPlan?.[0]?.december || "",
            ...
            console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
            const url =
                `/api/register/component/action/one/${[...selectedRowsForActions][0]}?token=${token}`;
            fetch(url, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload), // Direkt obje – array yapma!
...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/fin/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch(`/api/register/fin/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
    ...
    if (!showDeletedAction) {
      ...
      fetch(
        `/api/register/component/action/all/delete?token=${token}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      fetch(
        `/api/register/component/action/all/undelete?token=${token}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
...
const archiveData = (id) => {
  ...
  if (showArchived) {
    fetch(`/api/register/fin/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ids: [...selectedRows],
    ...
    fetch(`/api/register/fin/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
        ...
        <div className="flex-1 p-8 bg-gradient-to-br from-blue-50/50 to-white h-full overflow-y-auto">
          {selectedOption === "e-chart" ? (
            ...
            <div className="bg-white !rounded-button shadow-lg overflow-hidden">
              ...
              <div className="p-6 border-b border-blue-100 flex items-center">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  >
                    ...
                    {!showAction ? "Add Finding" : "Add Action"}
                  </button>
              ...
              <div className="overflow-x-auto max-h-[75vh] overflow-y-auto">
                <table>
                  <FHeaders activeHeader={activeHeader} />
                  <FBody
...
{showModal &&
  (activeHeader ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        ...
        <div className="px-8 py-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-800">
              {modalMode === "add" ? "Add New Finding" : "Edit Finding"}
            </h3>
          </div>
        ...
        <div className="px-8 py-6 space-y-6">
          <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest">Finding Details</p>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-6">
              ...
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Category Of Finding</label>
                <select
                  value={formData.categoryOfFinding || ""}
                  onChange={(e) => {
                    console.log("Select onChange tetiklendi! Yeni value:", e.target.value);
                    handleFormChange("categoryOfFinding", e.target.value);
                  }}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
                  {dropdownData?.categoryOfFinding?.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.value}
                    </option>
                  ))}
              ...
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Source Of Finding</label>
                <select
                  value={formData.sourceOfFinding || ""}
                  onChange={(e) => {
                    console.log("Select onChange tetiklendi! Yeni value:", e.target.value);
                    handleFormChange("sourceOfFinding", e.target.value);
                  }}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
                  {dropdownData?.sourceOfFinding?.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.value}
                    </option>
                  ))}
            ...
            <div className="space-y-6">
              ...
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Description</label>
                <input
              ...
              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Containment Action</label>
                <input
                  value={formData.containmentAction}
                  onChange={(e) => handleFormChange("containmentAction", e.target.value)}
                  type="text"
                  placeholder="Enter containment action..."
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                />
              </div>

              <div className="group">
                <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Root Causes</label>
                <input
                  value={formData.rootCauses}
                  onChange={(e) => handleFormChange("rootCauses", e.target.value)}
                  type="text"
    ...
    // Action modalı – önceki dönüşümlerle aynı stil
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        {/* Header */}
        <div className="px-8 py-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-2xl">
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={flogHelpContent} />
    </div>
  );
};
