...
import { docHelpContent } from "./utils/helpContents.js";
...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/doc/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      fetch(`/api/register/doc/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids: [...selectedRows] }),
      })
...
const archiveData = (id) => {
  const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
  if (showArchived) {
    fetch(`/api/register/doc/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: [...selectedRows] }),
    })
    ...
    fetch(`/api/register/doc/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: [...selectedRows] }),
    })
    ...
    >
      ...
      />
                ...
                <div className="ml-auto flex items-center gap-2">
                  <ExportButton moduleKey="doc-reg" />
                  <button
              ...
              <div className="overflow-x-auto max-h-[75vh] overflow-y-auto">
                <table>
                  <DocHeaders activeHeader={activeHeader} />
                  <DocBody
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        ...
        <div className="px-8 py-6 space-y-6">
          <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest">Document Details</p>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {/* Origin */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Origin</label>
              <select
            ...
            {/* Conditional: Department / Function Name */}
              <div className="group">
                ...
                <select
                  value={formData.depntFunctionName?.id || ""}
                  onChange={(e) => handleFormChange("depntFunctionName.id", e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
            ...
            {/* Conditional: Type */}
              <div className="group">
                ...
                <select
                  ...
                  onChange={(e) => handleFormChange("type.id", e.target.value)}
                  className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
                >
                  <option value="">Select</option>
            ...
            {/* Revision Number */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Revision Number</label>
              <input
                value={formData.revNumber}
                onChange={(e) => handleFormChange("revNumber", e.target.value)}
                type="text"
            ...
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Next Next Review Date</label>
              <input
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={docHelpContent} />
    </div>
  );
};
