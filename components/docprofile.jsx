...
const DocProfile = () => {
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      ...
      if (!showAction) {
        ...
        console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
        const url =
          "/api/register/doc/one/" + selectedTable[0].id;
        fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload), // Direkt obje – array yapma!
  ...
  const handleDeleteConfirm = () => {
    if (activeHeader) {
      if (!showDeleted) {
        fetch("/api/register/doc/all/delete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
        ...
        fetch("/api/register/doc/all/undelete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
  ...
  const archiveData = (id) => {
    if (showArchived) {
      fetch("/api/register/doc/all/unarchive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch("/api/register/doc/all/archive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
