...
const EiProfile = () => {
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      ...
      if (!showAction) {
        ...
        console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
        const url =
          "/api/register/ei/one/" + selectedTable[0].id;
        fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload), // Direkt obje – array yapma!
  ...
  const handleDeleteConfirm = () => {
    if (activeHeader) {
      if (!showDeleted) {
        fetch("/api/register/ei/all/delete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
        ...
        fetch("/api/register/ei/all/undelete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
  ...
  const archiveData = (id) => {
    if (showArchived) {
      fetch("/api/register/ei/all/unarchive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch("/api/register/ei/all/archive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
