...
const EiProfile = () => {
  ...
  const saveRisk = () => {
    if (modalMode === "add") {
      if (!showAction) {
        ...
        fetch("http://localhost:8000/api/register/ei/one", {
          method: "POST",
      ...
      if (!showAction) {
        ...
        console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
        const url =
          "http://localhost:8000/api/register/ei/one/" + selectedTable[0].id;
        fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload), // Direkt obje – array yapma!
  ...
  const handleDeleteConfirm = () => {
    if (activeHeader) {
      if (!showDeleted) {
        fetch("http://localhost:8000/api/register/ei/all/delete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
        ...
        fetch("http://localhost:8000/api/register/ei/all/undelete", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ids: [...selectedRows],
  ...
  const archiveData = (id) => {
    if (showArchived) {
      fetch("http://localhost:8000/api/register/ei/all/unarchive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
      ...
      fetch("http://localhost:8000/api/register/ei/all/archive", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleFormChange("certificateNo", e.target.value)
                        }
                        type="text"
                        className="w-full px-3 py-2 border border-gray-300 !rounded-button focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                      />
