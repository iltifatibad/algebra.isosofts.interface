...
import { eiHelpContent } from "./utils/helpContents.js";
    ...
    if (modalMode === "add") {
        if (!showAction) {
            const payload = {
                ...
                calibrationRequired: parseInt(formData.calibrationRequired),
        ...
        if (!showAction) {
            ...
            console.log("Gönderilen body:", payload); // Debug: Tam beklenen format mı?
            const url =
                `/api/register/ei/one/${selectedTable[0].id}?token=${token}`;
            fetch(url, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload), // Direkt obje – array yapma!
...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/ei/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ids: [...selectedRows],
...
const archiveData = (id) => {
  ...
  if (showArchived) {
    fetch(`/api/register/ei/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ids: [...selectedRows],
...
<div className="flex items-center gap-4 flex-wrap">
  ...
  >
    ...
    {!showAction ? "Add Equipment / Inventory" : "Add Action"}
  </button>
  ...
  <div className="flex items-center gap-2.5">
    ...
    >
      ...
      />
              ...
              <div className="overflow-x-auto max-h-[75vh] overflow-y-auto">
                <table>
                  <EiHeaders activeHeader={activeHeader} />
                  <EiBody
                ...
                <div className="space-y-6">
                  ...
                  <div className="group">
                    ...
                    >
                      ...
                      {dropdownData?.equipmentType?.map((item) => (
    ...
    {showDeleteModal && (
      <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
          ...
          <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={eiHelpContent} />
    </div>
  );
};

export default EiProfile;
