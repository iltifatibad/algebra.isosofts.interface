...
const getArchivedData = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/register/cus/all?status=archived&token=${token}`);
    if (!response.ok) {
...
const getDeletedData = async () => {
  ...
  try {
    const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
    const response = await fetch(`/api/register/cus/all?status=deleted&token=${token}`);
    if (!response.ok) {
...
const applyComputedFilters = (data) => {
  ...
  if (filters["_daysDifference"]?.trim()) {
    result = result.filter(row => {
      if (!row.registrationDate || !row.reviewDate) return false;
      const d = Math.ceil((new Date(row.reviewDate) - new Date()) / (1000 * 60 * 60 * 24));
      return `${d} Days`.toLowerCase().includes(filters["_daysDifference"].toLowerCase());
    });
  }
  if (filters["_evaluationStatus"]?.trim()) {
    result = result.filter(row => {
      if (!row.reviewDate) return "no".includes(filters["_evaluationStatus"].toLowerCase());
      const d = Math.ceil((new Date(row.reviewDate) - new Date()) / (1000 * 60 * 60 * 24));
      return (d >= 0 ? "yes" : "no").includes(filters["_evaluationStatus"].toLowerCase());
    });
  }
...
const getAll = async () => {
  setLoading(true);
  const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
  fetch(`/api/register/cus/all?token=${token}`)
    .then((response) => {
      if (!response.ok) {
            ...
            <tr className="bg-gray-50 border-b-2 border-blue-200">
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  ...
                  onChange={e => setFilters(prev => ({...prev, "reviewDate": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
...
<td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
  <input
    value={filters["_evaluationStatus"] || ""}
    onChange={e => setFilters(prev => ({...prev, "_evaluationStatus": e.target.value}))}
    placeholder="Filter..."
    className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
  />
</td>
...
{/* Reg Number */}
<td className="border border-gray-200 px-2 py-1 w-20" rowSpan={1}>
  <SoftBadge value={row.regNumber} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Scope 1 */}
<td className="border border-gray-200 px-2 py-1 w-20" rowSpan={1}>
  <SoftBadge value={row.scope1?.value} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Scope 2 */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.scope2?.value} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Scope 3 */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.scope3?.value} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Registration Date */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.registrationDate} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Next Review Date */}
<td className="border border-gray-200 px-2 py-1 w-28" rowSpan={1}>
  <SoftBadge value={row.reviewDate} color="bg-gray-100 text-black border border-gray-200" />
</td>
