...
const getArchivedData = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/register/tra/all?status=archived&token=${token}`);
    if (!response.ok) {
...
const getDeletedData = async () => {
  ...
  try {
    const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
    const response = await fetch(`/api/register/tra/all?status=deleted&token=${token}`);
    if (!response.ok) {
      ...
      <tbody className="text-xs">
            ...
            <tr className="bg-gray-50 border-b-2 border-blue-200">
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["tcln"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "tcln": e.target.value}))}
                  placeholder="Filtrele..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["clnumber"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "clnumber": e.target.value}))}
                  placeholder="Filtrele..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["trainingFrequency.value"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "trainingFrequency.value": e.target.value}))}
                  placeholder="Filtrele..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["ncd"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "ncd": e.target.value}))}
                  placeholder="Filtrele..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
...
{/* CL Name */}
<td className="border border-gray-200 px-2 py-1 w-20" rowSpan={1}>
  <SoftBadge value={row.tcln} color="bg-gray-100 text-black border border-gray-200" />
</td>

<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.clnumber} color="bg-gray-100 text-black border border-gray-200" />
</td>

<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.trainingFrequency.value} color="bg-gray-100 text-black border border-gray-200" />
</td>
...
{/* NCD */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.ncd} color="bg-gray-100 text-black border border-gray-200" />
</td>
...
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  {(() => {
    if (!row.ncd) return (
      <SoftBadge value="Not Valid" color="bg-gray-100 text-black border border-gray-200" />
    );
    const diffInDays = Math.ceil((new Date(row.ncd) - new Date()) / (1000 * 60 * 60 * 24));
