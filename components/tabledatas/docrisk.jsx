...
const getArchivedData = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/register/doc/all?status=archived&token=${token}`);
    if (!response.ok) {
...
const getDeletedData = async () => {
  ...
  try {
    const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
    const response = await fetch(`/api/register/doc/all?status=deleted&token=${token}`);
    if (!response.ok) {
...
const getAll = async () => {
  setLoading(true);
  const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
  fetch(`/api/register/doc/all?token=${token}`)
    .then((response) => {
      if (!response.ok) {
    ...
    return (
      <tbody className="text-xs">
            ...
            <tr className="bg-gray-50 border-b-2 border-blue-200">
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["origin?.value"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "origin?.value": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["revNumber"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "revNumber": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  ...
                  onChange={e => setFilters(prev => ({...prev, "issueDate": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["nextReviewDate"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "nextReviewDate": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
