...
const getArchivedData = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/register/moc/all?status=archived&token=${token}`);
    if (!response.ok) {
...
const getDeletedData = async () => {
  ...
  try {
    const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
    const response = await fetch(`/api/register/moc/all?status=deleted&token=${token}`);
    if (!response.ok) {
...
const getAll = async () => {
  setLoading(true);
  const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
  fetch(`/api/register/moc/all?token=${token}`)
    .then((response) => {
      if (!response.ok) {
    ...
    return (
      <tbody className="text-xs">
            ...
            <tr className="bg-gray-50 border-b-2 border-blue-200">
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["issuerDate"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "issuerDate": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["reasonOfChange"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "reasonOfChange": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              ...
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["changeDescription"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "changeDescription": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
              <td className="border border-gray-200 px-1 py-1 bg-gray-50 min-w-[60px]">
                <input
                  value={filters["risks"] || ""}
                  onChange={e => setFilters(prev => ({...prev, "risks": e.target.value}))}
                  placeholder="Filter..."
                  className="w-full text-[10px] border border-gray-300 rounded px-1 py-0.5 focus:outline-none focus:border-blue-400"
                />
              </td>
...
<td className="border border-gray-200 px-2 py-1 w-20" rowSpan={1}>
  <SoftBadge value={row.issuerDate} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Reason Of Change */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.reasonOfChange} color="bg-gray-100 text-black border border-gray-200" />
</td>
...
{/* Action */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.changeDescription} color="bg-gray-100 text-black border border-gray-200" />
</td>

{/* Risks */}
<td className="border border-gray-200 px-2 py-1 w-32" rowSpan={1}>
  <SoftBadge value={row.risks} color="bg-gray-100 text-black border border-gray-200" />
</td>
