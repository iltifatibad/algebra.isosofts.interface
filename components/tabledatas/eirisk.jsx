...
const EiBody = ({
  ...
  const getArchivedData = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/register/ei/all?status=archived",
      );
      if (!response.ok) {
  ...
  const getDeletedData = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/api/register/ei/all?status=deleted",
      );
      if (!response.ok) {
  ...
  const getAll = async () => {
    setLoading(true);
    fetch("http://localhost:8000/api/register/ei/all")
      .then((response) => {
        if (!response.ok) {
