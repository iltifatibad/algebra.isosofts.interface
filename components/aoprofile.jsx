...
const handleDeleteConfirm = () => {
  ...
  if (activeHeader) {
    if (!showDeleted) {
      fetch(`/api/register/aop/all/delete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      fetch(`/api/register/aop/all/undelete?token=${token}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ids: [...selectedRows] }),
      })
...
const archiveData = (id) => {
  const token = document.cookie.split("; ").find((r) => r.startsWith("auth_token="))?.split("=").slice(1).join("=") ?? "";
  if (showArchived) {
    fetch(`/api/register/aop/all/unarchive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: [...selectedRows] }),
    })
    ...
    fetch(`/api/register/aop/all/archive?token=${token}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: [...selectedRows] }),
    })
...
<div className="flex items-center gap-4 flex-wrap">
  ...
  <button
    ...
    className={`
      group relative px-6 py-2.5 rounded-xl font-medium text-sm tracking-wide
  ...
  >
    <i
      className={`fas ${showArchived ? "fa-undo" : "fa-archive"} text-base transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12`}
    />
  ...
  >
    <i
      className={`
        ...
        text-base transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6
      `}
    />
    {activeHeader
      ? showDeleted
  ...
  <div className="flex items-center gap-2.5">
    ...
    >
      <i
        className={`fas ${showArchived ? "fa-undo" : "fa-archive"} text-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12`}
      />
    </button>
    ...
    >
      <i
        className={`
          fas ${showDeleted || showDeletedAction ? "fa-trash-restore" : "fa-trash-can"} 
          text-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6
        `}
      />
    </button>
  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-100 modal-box">
        ...
        <div className="px-8 py-5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-blue-50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-800">
              {modalMode === "add" ? "Add New A&O Activity" : "Edit A&O Activity"}
            </h3>
          </div>
        ...
        <div className="px-8 py-6 space-y-5">
          <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest">Activity Details</p>

          <div className="grid md:grid-cols-2 gap-4">

            {/* Activity Description */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Activity Description</label>
              <select
            ...
            {/* Reviewed Premises */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Reviewed Premises</label>
              <select
                value={formData.reviewedPremises || ""}
                onChange={(e) => handleFormChange("reviewedPremises", e.target.value)}
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
              >
                <option value="">Select</option>
            ...
            {/* Auditor Inspector */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Auditor / Inspector</label>
              <input
                value={formData.auditorInspector}
                onChange={(e) => handleFormChange("auditorInspector", e.target.value)}
                type="text"
                placeholder="Enter auditor / inspector..."
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
              />
            </div>
            ...
            {/* Reviewed Process */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Reviewed Process</label>
              <select
                value={formData.reviewedProcess || ""}
                onChange={(e) => handleFormChange("reviewedProcess", e.target.value)}
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
              >
                <option value="">Select</option>
            ...
            {/* Auditee Inspectee */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">Auditee / Inspectee</label>
              <input
                value={formData.auditeeInspectee}
                onChange={(e) => handleFormChange("auditeeInspectee", e.target.value)}
                type="text"
                placeholder="Enter auditee / inspectee..."
                className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all"
              />
            </div>
            ...
            {/* A&O Criteria */}
            <div className="group">
              <label className="block text-xs font-medium text-gray-500 mb-1.5 group-focus-within:text-blue-500 transition-colors">A&O Criteria</label>
              <input
                value={formData.rtic}
        ...
        <div className="px-8 py-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50 rounded-b-2xl">
          ...
          <button onClick={saveRisk} className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl hover:from-blue-600 hover:to-blue-800 shadow-sm shadow-blue-200 transition-all">
            {modalMode === "add" ? "Add A&O Activity" : "Update A&O Activity"}
          </button>
        </div>
      </div>
    </div>
...
{showDeleteModal && (
  <div className="fixed inset-0 z-50 flex items-center justify-center modal-overlay">
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-100 modal-box">
      ...
      <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} helpData={aoHelpContent} />
    </div>
  );
};
